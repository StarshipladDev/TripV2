...
public class ClueGen {
  private int numC=(TripV2.total -1)*2;
  ...
  public ClueGen(Event[] e){
    ...
    count++;
    if (allTrue())
      clues[count]="Your mind is clear. You are sure all these events happened.";
    else
      clues[count]="Something's not right... you're sure at least one of these events didn't occur";
    count++;
    if (getHighestIndex()>=0)
      clues[count]="'"+events[getHighestIndex()].getName()+"' just happened. It's fresh in your mind.";
    else
      clues[count]="Did anything actually happen? You may have only just taken the tab...";
    count++;
    if (numberTrue()==0){
    ...
    count++;
    if (numberTrue()<=1){
      clues[count]="You should have figured it out by now. Consider the game lost, moron.";
    } else if (allTrue()){
      do{
        sequenceClue(2,events.length);
      } while(redundant());
    } else {
      clues[count]="'"+events[getTrueIndex(numberTrue()/2)].getName()+"' happened in the eariler portion of your trip. Or so they say...";
    }
    //end of clue 6
    count++;
    //if 4 things clues end here
    if(count<numC && numberTrue()>1){
      clues[count]="'"+events[getTrueIndex(numberTrue()/2+1,numberTrue())].getName()+"' happened in the middle or later portion of your trip. Or so they say...";
      count++;
      //end of 7
      if (allTrue()){
        clues[count]="'"+events[getTrueIndex(numberTrue()/2)].getName()+"' happened in the eariler portion of your trip. Or so they say...";
    ...
    if(count<numC && numberTrue()>1){
      ...
      if (allTrue()){
        ...
        clues[count]="Enough time has passed for "+numberTrue()+" things to have actually happened.";
      }
      count++;
    }//clues 9&10
    //end of 6
    if(count<numC && numberTrue()>1){
      if(numberTrue()<=events.length/2){
        do{
          ...
          do{
            fal=getFalseIndex();
            clues[count]="Josie says '"+events[fal].getName()+"' didn't happen. Time paradox is spoopy.";
          } while(redundant());
    ...
    if(count<numC&& numberTrue()>1){
      ...
      count++;
      if(numberTrue()<4)
        clues[count]="3 or FEWER things happened and you should know the order. Consider the game lost, mongoloid.";
        else if(allTrue()){
        do{
          laterThan();
        }while(redundant());
